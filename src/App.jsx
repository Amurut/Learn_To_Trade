...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      <header className="p-4 bg-slate-900 border-b border-slate-800 sticky top-0 z-50 shadow-2xl flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 shrink-0">
        ...
        <div className="grid grid-cols-3 gap-2 w-full sm:w-auto sm:flex sm:items-center sm:gap-6">
          <div className="text-center sm:text-right">
            <p className="text-[8px] text-slate-500 uppercase font-bold">Portfolio</p>
            <p className="text-white font-mono font-bold text-xs">${portfolioValue.toFixed(0)}</p>
          </div>
          <div className="text-center sm:text-right">
            <p className="text-[8px] text-slate-500 uppercase font-bold">Cash</p>
            <p className="text-green-400 font-mono font-bold text-xs">${balance.toFixed(0)}</p>
          </div>
          <div className="text-center sm:text-right">
            <p className="text-[8px] text-slate-500 uppercase font-bold">PNL</p>
            <p className={`font-mono font-bold text-xs ${unrealizedPnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {unrealizedPnl >= 0 ? '+' : ''}${unrealizedPnl.toFixed(0)}
            </p>
          </div>
          <button onClick={() => setIsGlossaryOpen(true)} className="col-span-3 sm:col-span-1 bg-slate-800 px-2 py-1 rounded text-[8px] font-bold hover:bg-slate-700">
            GLOSSARY
          </button>
        </div>
      </header>
      ...
      <main className="flex-grow p-4 container mx-auto w-full">
        {view === 'landing' ? (
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            {/* Chart Column: No fixed height wrapper */}
            <div className="w-full lg:w-2/3 block">
              ...
              {/* Chart component now manages its own internal height */}
              <TradingViewChart data={selectedChartData} symbol={selectedSymbol} />
            </div>

            {/* Sidebar: Top border on mobile, left border on desktop */}
            <div className="w-full lg:w-1/3 flex flex-col gap-6 pb-10 pt-6 border-t border-slate-800 lg:border-t-0 lg:pt-0 lg:border-l bg-slate-950/20 min-h-[300px] lg:pl-6">
              {/* Sidebar content stacks naturally */}
                ...
                {/* 2. Active Positions */}
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 shrink-0 min-h-[180px]">
                  <h3 className="text-[10px] font-bold text-slate-500 uppercase mb-3 tracking-widest">Active Positions</h3>
